...
public class CommandHandler extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        ...
        CommandPreConstructor commandPreConstructor = new CommandPreConstructor(event.getMessage().getContentRaw(), commandManager.getPrefix(), commandManager);
        if (commandPreConstructor.getBaseCommand() != null) {
            try {
                commandPreConstructor.getBaseCommand().execute(
                        new CommandEvent(event.getJDA(), (int) event.getResponseNumber(), event.getMessage(), commandPreConstructor),
                        event.getChannel(),
                        event.getMember(),
                        commandPreConstructor.getArgs());
            } catch (Exception exception) {
                commandManager.getLogger().warning("An error occurred while executing " + commandPreConstructor.getLabel());
